...
export async function mockApi(page: Page, options: MockApiOptions = {}) {
    ...
    floors: [
      {
        ...
        rooms: [
          { id: 'room-1', name: 'Office', points: [ [10,10], [45,10], [45,45], [10,45] ] },
          { id: 'room-2', name: 'Lab',    points: [ [55,10], [90,10], [90,45], [55,45] ] },
          { id: 'room-3', name: 'Hall',   points: [ [10,55], [90,55], [90,90], [10,90] ] }
        ]
      }
    ],
    devices: [],
  ...
  await page.route('**/api/state/**', (route) => {
    ...
    if (pathname.endsWith('/api/state/config'))
      return respond(demoConfig);
  ...
  if (options.stubWebSocket) {
    await page.addInitScript(() => {
      ...
      // @ts-ignore override in browser context
